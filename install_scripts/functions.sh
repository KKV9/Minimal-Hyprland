...
pacman_config() {
	...
	for line in $lines_to_edit; do
		if grep -q "^#$line" "$pacman_conf"; then
			...
			echo "Uncommented: $line" >&2
		else
			echo "$line is already uncommented." >&2
		fi
	done
	...
	if grep -q "^ParallelDownloads" "$pacman_conf" && ! grep -q "^ILoveCandy" "$pacman_conf"; then
		...
		echo "Added ILoveCandy below ParallelDownloads." >&2
	else
		echo "ILoveCandy already exists" >&2
	fi

	echo "Pacman.conf edited" >&2
